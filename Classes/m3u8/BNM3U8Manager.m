...
- (NSString *)getRootPath{
    return _config.downloadDstRootPath;
}
- (NSString *)folderByUrl:(NSString *)url{
    return [BNTool uuidWithUrl:url];
}
-(void)removeFolderByUrl:(NSString *)url{
    if (url==nil||url.length<=0) {
        return;
    }
    NSString *path= [self.config.downloadDstRootPath stringByAppendingPathComponent:[BNTool uuidWithUrl:url]];
    [BNFileManager.shareInstance removeFileWithPath:path];
}
- (bool)hasTask:(NSString *)url{
    if([_downloadOperationsMap.allKeys containsObject:url]){
        return YES;
    }
    return FALSE;
}
...
- (void)downloadVideoWithConfig:(BNM3U8DownloadConfig *)config progressBlock:(BNM3U8DownloadProgressBlock)progressBlock resultBlock:(BNM3U8DownloadResultBlock)resultBlock{
    ...
    NSLog(@"开始任务manager");
    BNM3U8DownloadOperation *operation =  [[BNM3U8DownloadOperation alloc]initWithConfig:config downloadDstRootPath:self.config.downloadDstRootPath sessionManager:self.sessionManager progressBlock:^(CGFloat progress,BNM3U8DownloadConfig * _Nonnull cfg) {
        if(progressBlock) progressBlock(progress,cfg.url);
    }resultBlock:^(NSError * _Nullable error, NSString * _Nullable localPlayUrlString,BNM3U8DownloadConfig * _Nonnull cfg) {
        ///下载回调
        if(resultBlock) resultBlock(error,localPlayUrlString,cfg.url);
